...
def export_kml(factory_coord=(39.976736, 32.562732)):
    ...
    # 1. Routes
    df_routes = pd.read_csv("all_routes.csv", converters={
        "latitude": lambda x: float(str(x).replace(",", ".")),
        "longitude": lambda x: float(str(x).replace(",", "."))
    })
    ...
    # 2. Pickup points
    df_pickups = pd.read_csv("pickup_points_result.csv", converters={
        "latitude": lambda x: float(str(x).replace(",", ".")),
        "longitude": lambda x: float(str(x).replace(",", "."))
    })
    ...
    df_homes = pd.read_csv("locations_all.csv", converters={
        "latitude": lambda x: float(str(x).replace(",", ".")),
        "longitude": lambda x: float(str(x).replace(",", "."))
    })
    for idx, row in df_homes.iterrows():
        if str(row["id"]).lower() == "factory":
            ...
            point.style.iconstyle.color = "ff999999"
